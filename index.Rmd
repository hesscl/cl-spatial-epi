...
sd <- SharedData$new(race_dens, ~race)
...
```{r raceeth1}
raceeth1 <- ggplot(sd, aes(x = race, y = percent, group = race, color = race)) + 
  geom_boxplot() +
...
```{r raceeth2}
raceeth2 <- ggplot(sd, aes(x = percent, y = nListings, group = race, color = race)) + 
  geom_smooth(se = F) + 
...
```{r m1coef, fig.dpi = 300, fig.height=6, fig.width=6}
#like a ropeladder, but bayes-y 
c1 <- coefINLA(m1, labeller = var_labeller) + 
  scale_x_continuous(limits = c(-60, 60)) +
  xlab("Exponentiated Coefficients")

ggplotly(c1, tooltip = c("var", "med")) %>%
  layout(margin = list(r = 125))
```
...
### Spatial RE Model posterior distributions (with 95% intervals)

```{r m2coef, fig.dpi = 300, fig.height=6, fig.width=6}
c2 <- coefINLA(m2, labeller = var_labeller) + 
  scale_x_continuous(limits = c(-60, 60)) +
  xlab("Exponentiated Coefficients")

ggplotly(c2, tooltip = c("var", "med")) %>%
  layout(margin = list(r = 125))
```
